...
assert.match(uiJs, /touchMoved\s*=\s*true/, 'Scrollgeste wird als bewegt markiert');
assert.match(uiJs, /if \(touchMoved\)[\s\S]*?return;/, 'Touch-Ende nach Scrollen löst kein Feld aus');
assert.match(uiJs, /best_custom_' \+ config\.rows \+ 'x' \+ config\.cols \+ 'x' \+ config\.mines/, 'Custom-Bestzeit ist dimensionsspezifisch');
